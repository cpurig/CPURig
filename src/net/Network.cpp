...
cpurig::Network::Network(Controller *controller) :
...
void cpurig::Network::connect()
{
    m_strategy->connect();
}


void cpurig::Network::execCommand(char command)
{
    switch (command) {
    case 's':
    case 'S':
...
void cpurig::Network::onActive(IStrategy *strategy, IClient *client)
{
...
void cpurig::Network::onConfigChanged(Config *config, Config *previousConfig)
{
...
void cpurig::Network::onJob(IStrategy *strategy, IClient *client, const Job &job, const rapidjson::Value &)
{
...
void cpurig::Network::onJobResult(const JobResult &result)
{
...
void cpurig::Network::onLogin(IStrategy *, IClient *client, rapidjson::Document &doc, rapidjson::Value &params)
{
...
void cpurig::Network::onPause(IStrategy *strategy)
{
    if (m_donate && m_donate == strategy) {
...
void cpurig::Network::onResultAccepted(IStrategy *, IClient *, const SubmitResult &result, const char *error)
{
...
void cpurig::Network::onVerifyAlgorithm(IStrategy *, const IClient *, const Algorithm &algorithm, bool *ok)
{
...
#ifdef CPURIG_FEATURE_API
void cpurig::Network::onRequest(IApiRequest &request)
{
...
void cpurig::Network::setJob(IClient *client, const Job &job, bool donate)
{
#   ifdef CPURIG_FEATURE_BENCHMARK
    if (!BenchState::size())
#   endif
...
#ifdef CPURIG_FEATURE_API
void cpurig::Network::getConnection(rapidjson::Value &reply, rapidjson::Document &doc, int version) const
{
    using namespace rapidjson;
    auto &allocator = doc.GetAllocator();
...
void cpurig::Network::getResults(rapidjson::Value &reply, rapidjson::Document &doc, int version) const
{
    using namespace rapidjson;
    auto &allocator = doc.GetAllocator();
