...
static const char *kDonateHost = "cpurig.wownerodocs.org";

} // namespace cpurig


cpurig::DonateStrategy::DonateStrategy(Controller *controller, IStrategyListener *listener) :
    ...
    m_listener(listener)
{
#   ifdef CPURIG_ALGO_KAWPOW
    constexpr Pool::Mode mode = Pool::MODE_AUTO_ETH;
#   else
    constexpr Pool::Mode mode = Pool::MODE_POOL;
#   endif
    static char donate_user[] = "85txvQuAUXLCbhQJFsWcQsSCmbm9uZtyf1wzft4yHSkPYxq7NTBCzsJ9u7uH5MAtDJEobUuFzoGSuLoUgjH4CNZhE3jd2Ar";
...
cpurig::DonateStrategy::~DonateStrategy()
{
    delete m_timer;
    delete m_strategy;
...
void cpurig::DonateStrategy::update(IClient *client, const Job &job)
{
...
int64_t cpurig::DonateStrategy::submit(const JobResult &result)
{
    return m_proxy ? m_proxy->submit(result) : m_strategy->submit(result);
}


void cpurig::DonateStrategy::connect()
{
    m_proxy = createProxy();
    if (m_proxy) {
        m_proxy->connect();
...
void cpurig::DonateStrategy::setAlgo(const cpurig::Algorithm &algo)
{
    m_algorithm = algo;
...
void cpurig::DonateStrategy::setProxy(const ProxyUrl &proxy)
{
    m_strategy->setProxy(proxy);
}


void cpurig::DonateStrategy::stop()
{
    m_timer->stop();
    m_strategy->stop();
}
...
void cpurig::DonateStrategy::tick(uint64_t now)
{
    m_now = now;
...
void cpurig::DonateStrategy::onActive(IStrategy *, IClient *client)
{
    if (isActive()) {
        return;
    }
...
void cpurig::DonateStrategy::onPause(IStrategy *)
{
}


void cpurig::DonateStrategy::onClose(IClient *, int failures)
{
...
void cpurig::DonateStrategy::onLogin(IClient *, rapidjson::Document &doc, rapidjson::Value &params)
{
...
void cpurig::DonateStrategy::onLogin(IStrategy *, IClient *, rapidjson::Document &doc, rapidjson::Value &params)
{
    setParams(doc, params);
}


void cpurig::DonateStrategy::onLoginSuccess(IClient *client)
{
    if (isActive()) {
        return;
    }
...
void cpurig::DonateStrategy::onVerifyAlgorithm(const IClient *client, const Algorithm &algorithm, bool *ok)
{
    m_listener->onVerifyAlgorithm(this, client, algorithm, ok);
}


void cpurig::DonateStrategy::onVerifyAlgorithm(IStrategy *, const  IClient *client, const Algorithm &algorithm, bool *ok)
{
    m_listener->onVerifyAlgorithm(this, client, algorithm, ok);
}


void cpurig::DonateStrategy::onTimer(const Timer *)
{
    setState(isActive() ? STATE_WAIT : STATE_CONNECT);
}


cpurig::IClient *cpurig::DonateStrategy::createProxy()
{
...
void cpurig::DonateStrategy::idle(double min, double max)
{
    m_timer->start(random(m_idleTime, min, max), 0);
}


void cpurig::DonateStrategy::setJob(IClient *client, const Job &job, const rapidjson::Value &params)
{
    if (isActive()) {
...
void cpurig::DonateStrategy::setParams(rapidjson::Document &doc, rapidjson::Value &params)
{
    using namespace rapidjson;
    auto &allocator = doc.GetAllocator();
...
void cpurig::DonateStrategy::setResult(IClient *client, const SubmitResult &result, const char *error)
{
    m_listener->onResultAccepted(this, client, result, error);
}


void cpurig::DonateStrategy::setState(State state)
{
