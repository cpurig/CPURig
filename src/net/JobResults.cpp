...
#if defined(CPURIG_FEATURE_OPENCL) || defined(CPURIG_FEATURE_CUDA)
...
{
public:
    CPURIG_DISABLE_COPY_MOVE_DEFAULT(JobResultsPrivate)
...
void cpurig::JobResults::done(const Job &job)
{
    submit(JobResult(job));
}


void cpurig::JobResults::setListener(IJobResultListener *listener, bool hwAES)
{
...
void cpurig::JobResults::submit(const Job &job, uint32_t nonce, const uint8_t *result)
{
    submit(JobResult(job, nonce, result));
}


void cpurig::JobResults::submit(const Job& job, uint32_t nonce, const uint8_t* result, const uint8_t* miner_signature)
{
    submit(JobResult(job, nonce, result, nullptr, nullptr, miner_signature));
}


void cpurig::JobResults::submit(const JobResult &result)
{
    assert(handler != nullptr);
...
#if defined(CPURIG_FEATURE_OPENCL) || defined(CPURIG_FEATURE_CUDA)
void cpurig::JobResults::submit(const Job &job, uint32_t *results, size_t count, uint32_t device_index)
{
    if (handler) {
