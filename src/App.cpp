...
cpurig::App::App(Process *process)
{
...
int cpurig::App::exec()
{
    if (!m_controller->isReady()) {
        LOG_EMERG("no valid configuration found, try https://cpurig.com/wizard");
...
void cpurig::App::onConsoleCommand(char command)
{
    if (command == 3) {
...
void cpurig::App::onSignal(int signum)
{
    switch (signum)
    {
    case SIGHUP:
