...
bool cpurig::NvmlLib::init(const char *fileName)
{
    if (!m_initialized) {
        m_loader      = fileName;
        m_ready       = dlopen() && load();
...
const char *cpurig::NvmlLib::lastError() noexcept
{
    return uv_dlerror(&nvmlLib);
}


void cpurig::NvmlLib::close()
{
    if (m_ready) {
        pNvmlShutdown();
    }
...
bool cpurig::NvmlLib::assign(std::vector<CudaDevice> &devices)
{
    uint32_t count = 0;
...
NvmlHealth cpurig::NvmlLib::health(nvmlDevice_t device)
{
    if (!device) {
        return {};
    }
