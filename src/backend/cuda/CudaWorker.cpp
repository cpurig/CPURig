...
cpurig::CudaWorker::CudaWorker(size_t id, const CudaLaunchData &data) :
...
cpurig::CudaWorker::~CudaWorker()
{
    delete m_runner;
}


void cpurig::CudaWorker::jobEarlyNotification(const Job &job)
{
    if (m_runner) {
...
bool cpurig::CudaWorker::selfTest()
{
    return m_runner != nullptr;
}


size_t cpurig::CudaWorker::intensity() const
{
    return m_runner ? m_runner->roundSize() : 0;
}


void cpurig::CudaWorker::start()
{
...
bool cpurig::CudaWorker::consumeJob()
{
    if (Nonce::sequence(Nonce::CUDA) == 0) {
        return false;
    }
...
void cpurig::CudaWorker::storeStats()
{
    if (!isReady()) {
        return;
    }
