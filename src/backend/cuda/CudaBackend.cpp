...
#ifdef CPURIG_FEATURE_NVML
#include "backend/cuda/wrappers/NvmlLib.h"

namespace cpurig { static const char *kNvmlLabel = "NVML"; }
#endif
...
const char *cpurig::cuda_tag()
{
    return Tags::nvidia();
}


cpurig::CudaBackend::CudaBackend(Controller *controller) :
...
cpurig::CudaBackend::~CudaBackend()
{
    delete d_ptr;
...
bool cpurig::CudaBackend::isEnabled() const
{
    return d_ptr->controller->config()->cuda().isEnabled() && CudaLib::isInitialized() && !d_ptr->devices.empty();;
}


bool cpurig::CudaBackend::isEnabled(const Algorithm &algorithm) const
{
    return !d_ptr->controller->config()->cuda().threads().get(algorithm).isEmpty();
}


const cpurig::Hashrate *cpurig::CudaBackend::hashrate() const
{
    return d_ptr->workers.hashrate();
}


const cpurig::String &cpurig::CudaBackend::profileName() const
{
    return d_ptr->profileName;
}


const cpurig::String &cpurig::CudaBackend::type() const
{
    return kType;
}


void cpurig::CudaBackend::execCommand(char)
{
}


void cpurig::CudaBackend::prepare(const Job &job)
{
    if (d_ptr) {
...
void cpurig::CudaBackend::printHashrate(bool details)
{
    if (!details || !hashrate()) {
        return;
    }
...
void cpurig::CudaBackend::printHealth()
{
#   ifdef CPURIG_FEATURE_NVML
    d_ptr->printHealth();
#   endif
...
void cpurig::CudaBackend::setJob(const Job &job)
{
...
void cpurig::CudaBackend::start(IWorker *worker, bool ready)
{
    mutex.lock();
...
bool cpurig::CudaBackend::tick(uint64_t ticks)
{
...
#ifdef CPURIG_FEATURE_API
rapidjson::Value cpurig::CudaBackend::toJSON(rapidjson::Document &doc) const
{
...
void cpurig::CudaBackend::handleRequest(IApiRequest &)
{
}
#endif
