...
cpurig::CudaThread::CudaThread(const rapidjson::Value &value)
{
    if (!value.IsObject()) {
        return;
    }
...
cpurig::CudaThread::CudaThread(uint32_t index, nvid_ctx *ctx) :
...
bool cpurig::CudaThread::isEqual(const CudaThread &other) const
{
...
rapidjson::Value cpurig::CudaThread::toJSON(rapidjson::Document &doc) const
{
