...
cpurig::CudaKawPowRunner::CudaKawPowRunner(size_t index, const CudaLaunchData &data) :
    CudaBaseRunner(index, data)
{
}


bool cpurig::CudaKawPowRunner::run(uint32_t /*startNonce*/, uint32_t *rescount, uint32_t *resnonce)
{
    return callWrapper(CudaLib::kawPowHash(m_ctx, m_jobBlob, m_target, rescount, resnonce, &m_skippedHashes));
}


bool cpurig::CudaKawPowRunner::set(const Job &job, uint8_t *blob)
{
...
void cpurig::CudaKawPowRunner::jobEarlyNotification(const Job&)
{
