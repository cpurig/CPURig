...
cpurig::CudaBaseRunner::CudaBaseRunner(size_t id, const CudaLaunchData &data) :
    m_data(data),
    m_threadId(id)
{
}
...
cpurig::CudaBaseRunner::~CudaBaseRunner()
{
    CudaLib::release(m_ctx);
}


bool cpurig::CudaBaseRunner::init()
{
...
bool cpurig::CudaBaseRunner::set(const Job &job, uint8_t *blob)
{
    m_height = job.height();
    m_target = job.target();
...
size_t cpurig::CudaBaseRunner::intensity() const
{
    return m_data.thread.threads() * m_data.thread.blocks();
}


bool cpurig::CudaBaseRunner::callWrapper(bool result) const
{
    if (!result) {
