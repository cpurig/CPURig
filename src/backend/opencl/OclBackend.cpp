...
namespace cpurig { static const char *kAdlLabel = "ADL"; }
#endif
...
const char *cpurig::ocl_tag()
{
    return Tags::opencl();
}


cpurig::OclBackend::OclBackend(Controller *controller) :
...
bool cpurig::OclBackend::isEnabled() const
{
    return d_ptr->controller->config()->cl().isEnabled() && OclLib::isInitialized() && d_ptr->platform.isValid() && !d_ptr->devices.empty();
}


bool cpurig::OclBackend::isEnabled(const Algorithm &algorithm) const
{
    return !d_ptr->controller->config()->cl().threads().get(algorithm).isEmpty();
}


const cpurig::Hashrate *cpurig::OclBackend::hashrate() const
{
    return d_ptr->workers.hashrate();
}


const cpurig::String &cpurig::OclBackend::profileName() const
{
    return d_ptr->profileName;
}


const cpurig::String &cpurig::OclBackend::type() const
{
    return kType;
}


void cpurig::OclBackend::execCommand(char)
{
}


void cpurig::OclBackend::prepare(const Job &job)
{
    if (d_ptr) {
...
void cpurig::OclBackend::printHashrate(bool details)
{
    if (!details || !hashrate()) {
        return;
    }
...
void cpurig::OclBackend::printHealth()
{
#   ifdef CPURIG_FEATURE_ADL
    d_ptr->printHealth();
#   endif
...
void cpurig::OclBackend::setJob(const Job &job)
{
...
void cpurig::OclBackend::start(IWorker *worker, bool ready)
{
    mutex.lock();
...
bool cpurig::OclBackend::tick(uint64_t ticks)
{
...
#ifdef CPURIG_FEATURE_API
rapidjson::Value cpurig::OclBackend::toJSON(rapidjson::Document &doc) const
{
...
void cpurig::OclBackend::handleRequest(IApiRequest &)
{
}
#endif
