...
void cpurig::FindSharesKernel::enqueue(cl_command_queue queue, size_t threads)
{
...
void cpurig::FindSharesKernel::setArgs(cl_mem hashes, cl_mem shares)
{
...
void cpurig::FindSharesKernel::setTarget(uint64_t target)
{
    setArg(1, sizeof(uint64_t), &target);
}


void cpurig::FindSharesKernel::setNonce(uint32_t nonce)
{
