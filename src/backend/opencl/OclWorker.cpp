...
cpurig::OclWorker::OclWorker(size_t id, const OclLaunchData &data) :
...
cpurig::OclWorker::~OclWorker()
{
    delete m_runner;
}


void cpurig::OclWorker::jobEarlyNotification(const Job &job)
{
    if (m_runner) {
...
bool cpurig::OclWorker::selfTest()
{
    return m_runner != nullptr;
}


size_t cpurig::OclWorker::intensity() const
{
    return m_runner ? m_runner->roundSize() : 0;
}


void cpurig::OclWorker::start()
{
...
bool cpurig::OclWorker::consumeJob()
{
    if (Nonce::sequence(Nonce::OPENCL) == 0) {
        return false;
    }
...
void cpurig::OclWorker::storeStats(uint64_t t)
{
    if (!isReady()) {
        return;
    }
