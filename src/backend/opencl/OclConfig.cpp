...
#ifndef CPURIG_OS_APPLE
cpurig::OclConfig::OclConfig() : m_platformVendor(kAMD) {}
#else
cpurig::OclConfig::OclConfig() = default;
#endif


cpurig::OclPlatform cpurig::OclConfig::platform() const
{
...
rapidjson::Value cpurig::OclConfig::toJSON(rapidjson::Document &doc) const
{
...
std::vector<cpurig::OclLaunchData> cpurig::OclConfig::get(const Miner *miner, const Algorithm &algorithm, const OclPlatform &platform, const std::vector<OclDevice> &devices) const
{
...
void cpurig::OclConfig::read(const rapidjson::Value &value)
{
    if (value.IsObject()) {
...
void cpurig::OclConfig::generate()
{
    ...
    count += cpurig::generate<Algorithm::CN>(m_threads, devices);
    count += cpurig::generate<Algorithm::CN_LITE>(m_threads, devices);
    count += cpurig::generate<Algorithm::CN_HEAVY>(m_threads, devices);
    count += cpurig::generate<Algorithm::CN_PICO>(m_threads, devices);
    count += cpurig::generate<Algorithm::CN_FEMTO>(m_threads, devices);
    count += cpurig::generate<Algorithm::RANDOM_X>(m_threads, devices);
    count += cpurig::generate<Algorithm::KAWPOW>(m_threads, devices);

    m_shouldSave = count > 0;
}


void cpurig::OclConfig::setDevicesHint(const char *devicesHint)
{
...
#ifndef CPURIG_OS_APPLE
void cpurig::OclConfig::setPlatform(const rapidjson::Value &platform)
{
