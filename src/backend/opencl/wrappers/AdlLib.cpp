...
const char *cpurig::AdlLib::lastError() noexcept
{
    return uv_dlerror(&adlLib);
}


void cpurig::AdlLib::close()
{
    if (m_ready) {
...
AdlHealth cpurig::AdlLib::health(const OclDevice &device)
{
...
bool cpurig::AdlLib::dlopen()
{
    return uv_dlopen("atiadlxx.dll", &adlLib) == 0;
}


bool cpurig::AdlLib::load()
{
    try {
