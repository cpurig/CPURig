...
cpurig::OclThread::OclThread(const rapidjson::Value &value)
{
    if (!value.IsObject()) {
        return;
    }
...
bool cpurig::OclThread::isEqual(const OclThread &other) const
{
...
rapidjson::Value cpurig::OclThread::toJSON(rapidjson::Document &doc) const
{
