...
cpurig::OclRyoRunner::OclRyoRunner(size_t index, const OclLaunchData &data) : OclBaseRunner(index, data)
{
...
cpurig::OclRyoRunner::~OclRyoRunner()
{
    delete m_cn00;
    delete m_cn0;
    delete m_cn1;
...
size_t cpurig::OclRyoRunner::bufferSize() const
{
    return OclBaseRunner::bufferSize() + align(data().algorithm.l3() * m_intensity) + align(200 * m_intensity);
}


void cpurig::OclRyoRunner::run(uint32_t nonce, uint32_t *hashOutput)
{
...
void cpurig::OclRyoRunner::set(const Job &job, uint8_t *blob)
{
...
void cpurig::OclRyoRunner::build()
{
    OclBaseRunner::build();
