...
cpurig::OclSharedData &cpurig::OclSharedState::get(uint32_t index)
{
    return map[index];
}


void cpurig::OclSharedState::release()
{
    for (auto &kv : map) {
        kv.second.release();
    }
...
void cpurig::OclSharedState::start(const std::vector<OclLaunchData> &threads, const Job &job)
{
    assert(map.empty());
