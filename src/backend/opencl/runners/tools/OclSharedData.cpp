...
cl_mem cpurig::OclSharedData::createBuffer(cl_context context, size_t size, size_t &offset, size_t limit)
{
...
uint64_t cpurig::OclSharedData::adjustDelay(size_t /*id*/)
{
    if (m_threads < 2) {
        return 0;
    }
...
uint64_t cpurig::OclSharedData::resumeDelay(size_t /*id*/)
{
    if (m_threads < 2) {
        return 0;
    }
...
void cpurig::OclSharedData::release()
{
...
void cpurig::OclSharedData::setResumeCounter(uint32_t value)
{
    if (m_threads < 2) {
        return;
    }
...
void cpurig::OclSharedData::setRunTime(uint64_t time)
{
...
#ifdef CPURIG_ALGO_RANDOMX
cl_mem cpurig::OclSharedData::dataset() const
{
    if (!m_dataset) {
...
void cpurig::OclSharedData::createDataset(cl_context ctx, const Job &job, bool host)
{
    if (m_dataset) {
        return;
    }
