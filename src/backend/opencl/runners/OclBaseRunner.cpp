...
cpurig::OclBaseRunner::OclBaseRunner(size_t id, const OclLaunchData &data) :
    m_ctx(data.ctx),
...
#   ifdef CPURIG_STRICT_OPENCL_CACHE
    m_deviceKey += ":";
...
cpurig::OclBaseRunner::~OclBaseRunner()
{
...
size_t cpurig::OclBaseRunner::bufferSize() const
{
    return align(Job::kMaxBlobSize) + align(sizeof(cl_uint) * 0x100);
}


uint32_t cpurig::OclBaseRunner::deviceIndex() const
{
    return data().thread.index();
}


void cpurig::OclBaseRunner::build()
{
...
void cpurig::OclBaseRunner::init()
{
...
cl_mem cpurig::OclBaseRunner::createSubBuffer(cl_mem_flags flags, size_t size)
{
...
size_t cpurig::OclBaseRunner::align(size_t size) const
{
    return VirtualMemory::align(size, m_align);
}


void cpurig::OclBaseRunner::enqueueReadBuffer(cl_mem buffer, cl_bool blocking_read, size_t offset, size_t size, void *ptr)
{
...
void cpurig::OclBaseRunner::enqueueWriteBuffer(cl_mem buffer, cl_bool blocking_write, size_t offset, size_t size, const void *ptr)
{
...
void cpurig::OclBaseRunner::finalize(uint32_t *hashOutput)
{
