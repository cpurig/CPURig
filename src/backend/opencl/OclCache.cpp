...
cl_program cpurig::OclCache::build(const IOclRunner *runner)
{
...
#       ifdef _WIN32
        fileName = prefix() + "\\cpurig\\.cache\\" + cacheKey(runner) + ".bin";
#       else
...
std::string cpurig::OclCache::cacheKey(const char *deviceKey, const char *options, const char *source)
{
...
std::string cpurig::OclCache::cacheKey(const IOclRunner *runner)
{
    return cacheKey(runner->deviceKey(), runner->buildOptions(), runner->source());
}


void cpurig::OclCache::save(cl_program program, const std::string &fileName)
{
    size_t size = 0;
