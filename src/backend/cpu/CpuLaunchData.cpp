...
cpurig::CpuLaunchData::CpuLaunchData(const Miner *miner, const Algorithm &algorithm, const CpuConfig &config, const CpuThread &thread, size_t threads, const std::vector<int64_t>& affinities) :
    algorithm(algorithm),
...
bool cpurig::CpuLaunchData::isEqual(const CpuLaunchData &other) const
{
...
cpurig::CnHash::AlgoVariant cpurig::CpuLaunchData::av() const
{
    if (intensity <= 2) {
...
const char *cpurig::CpuLaunchData::tag()
{
    return cpu_tag();
}
