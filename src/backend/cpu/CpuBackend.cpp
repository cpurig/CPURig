...
const char *cpurig::backend_tag(uint32_t backend)
{
...
const char *cpurig::cpu_tag()
{
    return Tags::cpu();
}


cpurig::CpuBackend::CpuBackend(Controller *controller) :
...
cpurig::CpuBackend::~CpuBackend()
{
    delete d_ptr;
}


bool cpurig::CpuBackend::isEnabled() const
{
    return d_ptr->controller->config()->cpu().isEnabled();
}


bool cpurig::CpuBackend::isEnabled(const Algorithm &algorithm) const
{
    return algorithm.isValid() && !d_ptr->controller->config()->cpu().threads().get(algorithm).isEmpty();
}


bool cpurig::CpuBackend::tick(uint64_t ticks)
{
    return d_ptr->workers.tick(ticks);
}


const cpurig::Hashrate *cpurig::CpuBackend::hashrate() const
{
    return d_ptr->workers.hashrate();
}


const cpurig::String &cpurig::CpuBackend::profileName() const
{
    return d_ptr->profileName;
}


const cpurig::String &cpurig::CpuBackend::type() const
{
    return kType;
}


void cpurig::CpuBackend::prepare(const Job &nextJob)
{
#   ifdef CPURIG_ALGO_ARGON2
...
void cpurig::CpuBackend::printHashrate(bool details)
{
    if (!details || !hashrate()) {
        return;
    }
...
void cpurig::CpuBackend::printHealth()
{
}


void cpurig::CpuBackend::setJob(const Job &job)
{
    if (!isEnabled()) {
        return stop();
    }
...
void cpurig::CpuBackend::start(IWorker *worker, bool ready)
{
    mutex.lock();
...
#ifdef CPURIG_FEATURE_API
rapidjson::Value cpurig::CpuBackend::toJSON(rapidjson::Document &doc) const
{
...
void cpurig::CpuBackend::handleRequest(IApiRequest &request)
{
...
#ifdef CPURIG_FEATURE_BENCHMARK
cpurig::Benchmark *cpurig::CpuBackend::benchmark() const
{
    return d_ptr->benchmark.get();
}


void cpurig::CpuBackend::printBenchProgress() const
{
    if (d_ptr->benchmark) {
