...
cpurig::HwlocCpuInfo::HwlocCpuInfo()
{
...
#   if defined(CPURIG_OS_MACOS) && defined(CPURIG_ARM)
...
bool cpurig::HwlocCpuInfo::membind(hwloc_const_bitmap_t nodeset)
{
...
cpurig::CpuThreads cpurig::HwlocCpuInfo::threads(const Algorithm &algorithm, uint32_t limit) const
{
#   ifndef CPURIG_ARM
    if (L2() == 0 && L3() == 0) {
...
cpurig::CpuThreads cpurig::HwlocCpuInfo::allThreads(const Algorithm &algorithm, uint32_t limit) const
{
    CpuThreads threads;
    threads.reserve(m_threads);
...
void cpurig::HwlocCpuInfo::processTopLevelCache(hwloc_obj_t cache, const Algorithm &algorithm, CpuThreads &threads, size_t limit) const
{
#   ifndef CPURIG_ARM
...
void cpurig::HwlocCpuInfo::setThreads(size_t threads)
{
    if (!threads) {
        return;
    }
