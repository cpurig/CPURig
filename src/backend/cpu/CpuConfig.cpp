...
#ifdef CPURIG_FEATURE_ASM
const char *CpuConfig::kAsm                 = "asm";
#endif

#ifdef CPURIG_ALGO_ARGON2
...
bool cpurig::CpuConfig::isHwAES() const
{
    return (m_aes == AES_AUTO ? (Cpu::info()->hasAES() ? AES_HW : AES_SOFT) : m_aes) == AES_HW;
}


rapidjson::Value cpurig::CpuConfig::toJSON(rapidjson::Document &doc) const
{
...
size_t cpurig::CpuConfig::memPoolSize() const
{
    return m_memoryPool < 0 ? std::max(Cpu::info()->threads(), Cpu::info()->L3() >> 21) : m_memoryPool;
}


std::vector<cpurig::CpuLaunchData> cpurig::CpuConfig::get(const Miner *miner, const Algorithm &algorithm) const
{
...
void cpurig::CpuConfig::read(const rapidjson::Value &value)
{
    if (value.IsObject()) {
...
void cpurig::CpuConfig::generate()
{
    ...
    count += cpurig::generate<Algorithm::CN>(m_threads, m_limit);
    count += cpurig::generate<Algorithm::CN_LITE>(m_threads, m_limit);
    count += cpurig::generate<Algorithm::CN_HEAVY>(m_threads, m_limit);
    count += cpurig::generate<Algorithm::CN_PICO>(m_threads, m_limit);
    count += cpurig::generate<Algorithm::CN_FEMTO>(m_threads, m_limit);
    count += cpurig::generate<Algorithm::RANDOM_X>(m_threads, m_limit);
    count += cpurig::generate<Algorithm::ARGON2>(m_threads, m_limit);
    count += cpurig::generate<Algorithm::GHOSTRIDER>(m_threads, m_limit);

    m_shouldSave |= count > 0;
}


void cpurig::CpuConfig::setAesMode(const rapidjson::Value &value)
{
    if (value.IsBool()) {
...
void cpurig::CpuConfig::setHugePages(const rapidjson::Value &value)
{
    if (value.IsBool()) {
...
void cpurig::CpuConfig::setMemoryPool(const rapidjson::Value &value)
{
    if (value.IsBool()) {
