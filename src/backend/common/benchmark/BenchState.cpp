...
bool cpurig::BenchState::isDone()
{
    return d_ptr == nullptr;
}


uint32_t cpurig::BenchState::size()
{
    return d_ptr ? d_ptr->size : 0U;
}


uint64_t cpurig::BenchState::referenceHash(const Algorithm &algo, uint32_t size, uint32_t threads)
{
    uint64_t hash = 0;
...
uint64_t cpurig::BenchState::start(size_t threads, const IBackend *backend)
{
...
void cpurig::BenchState::destroy()
{
    delete d_ptr;
    d_ptr = nullptr;
}
...
void cpurig::BenchState::init(IBenchListener *listener, uint32_t size)
{
    assert(d_ptr == nullptr);
...
void cpurig::BenchState::setSize(uint32_t size)
{
    assert(d_ptr != nullptr);
