...
cpurig::Benchmark::Benchmark(size_t workers, const IBackend *backend) :
    m_backend(backend),
    m_workers(workers)
{
}
...
bool cpurig::Benchmark::finish(uint64_t totalHashCount)
{
...
void cpurig::Benchmark::start()
{
    m_startTime = BenchState::start(m_workers, m_backend);
}


void cpurig::Benchmark::printProgress() const
{
