...
cpurig::Hashrate::Hashrate(size_t threads) :
    m_threads(threads + 1)
{
...
double cpurig::Hashrate::average() const
{
...
const char *cpurig::Hashrate::format(double h, char *buf, size_t size)
{
    return ::format(h, buf, size);
}


rapidjson::Value cpurig::Hashrate::normalize(double d)
{
...
rapidjson::Value cpurig::Hashrate::toJSON(rapidjson::Document &doc) const
{
...
rapidjson::Value cpurig::Hashrate::toJSON(size_t threadId, rapidjson::Document &doc) const
{
    using namespace rapidjson;
    auto &allocator = doc.GetAllocator();
...
double cpurig::Hashrate::hashrate(size_t index, size_t ms) const
{
    ...
    const uint64_t timeStampLimit = cpurig::Chrono::steadyMSecs() - ms;
...
void cpurig::Hashrate::addData(size_t index, uint64_t count, uint64_t timestamp)
{
