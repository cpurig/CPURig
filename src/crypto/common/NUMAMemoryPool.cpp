...
cpurig::NUMAMemoryPool::NUMAMemoryPool(size_t size, bool hugePages) :
...
cpurig::NUMAMemoryPool::~NUMAMemoryPool()
{
    for (auto kv : m_map) {
        delete kv.second;
    }
...
bool cpurig::NUMAMemoryPool::isHugePages(uint32_t node) const
{
    if (!m_size) {
        return false;
    }
...
uint8_t *cpurig::NUMAMemoryPool::get(size_t size, uint32_t node)
{
    if (!m_size) {
        return nullptr;
    }
...
void cpurig::NUMAMemoryPool::release(uint32_t node)
{
...
cpurig::IMemoryPool *cpurig::NUMAMemoryPool::get(uint32_t node) const
{
    return m_map.count(node) ? m_map.at(node) : nullptr;
}


cpurig::IMemoryPool *cpurig::NUMAMemoryPool::getOrCreate(uint32_t node) const
{
    auto pool = get(node);
    if (!pool) {
