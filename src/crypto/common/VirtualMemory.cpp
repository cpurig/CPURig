...
cpurig::VirtualMemory::VirtualMemory(size_t size, bool hugePages, bool oneGbPages, bool usePool, uint32_t node, size_t alignSize) :
...
cpurig::VirtualMemory::~VirtualMemory()
{
    if (!m_scratchpad) {
        return;
    }
...
cpurig::HugePagesInfo cpurig::VirtualMemory::hugePages() const
{
    return { this };
}


#ifndef CPURIG_FEATURE_HWLOC
uint32_t cpurig::VirtualMemory::bindToNUMANode(int64_t)
{
    return 0;
}
#endif
...
void cpurig::VirtualMemory::destroy()
{
    delete pool;
}


void cpurig::VirtualMemory::init(size_t poolSize, size_t hugePageSize)
{
    if (!pool) {
        osInit(hugePageSize);
    }
