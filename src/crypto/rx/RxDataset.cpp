...
cpurig::RxDataset::RxDataset(bool hugePages, bool oneGbPages, bool cache, RxConfig::Mode mode, uint32_t node) :
    m_mode(mode),
    m_node(node)
{
...
cpurig::RxDataset::RxDataset(RxCache *cache) :
    m_node(0),
    m_cache(cache)
{
}
...
bool cpurig::RxDataset::init(const Buffer &seed, uint32_t numThreads, int priority)
{
...
bool cpurig::RxDataset::isHugePages() const
{
    return m_memory && m_memory->isHugePages();
}


bool cpurig::RxDataset::isOneGbPages() const
{
    return m_memory && m_memory->isOneGbPages();
}


cpurig::HugePagesInfo cpurig::RxDataset::hugePages(bool cache) const
{
...
size_t cpurig::RxDataset::size(bool cache) const
{
    size_t size = 0;
...
uint8_t *cpurig::RxDataset::tryAllocateScrathpad()
{
...
void *cpurig::RxDataset::raw() const
{
    return m_dataset ? randomx_get_dataset_memory(m_dataset) : nullptr;
}


void cpurig::RxDataset::setRaw(const void *raw)
{
    if (!m_dataset) {
        return;
    }
...
void cpurig::RxDataset::allocate(bool hugePages, bool oneGbPages)
{
