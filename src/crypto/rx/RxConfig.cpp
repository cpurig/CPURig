...
bool cpurig::RxConfig::read(const rapidjson::Value &value)
{
    if (value.IsObject()) {
...
rapidjson::Value cpurig::RxConfig::toJSON(rapidjson::Document &doc) const
{
...
#ifdef CPURIG_FEATURE_HWLOC
std::vector<uint32_t> cpurig::RxConfig::nodeset() const
{
    if (!m_nodeset.empty()) {
        return m_nodeset;
    }
...
const char *cpurig::RxConfig::modeName() const
{
    return modeNames[m_mode];
}


uint32_t cpurig::RxConfig::threads(uint32_t limit) const
{
    if (m_threads > 0) {
        return m_threads;
    }
...
#ifdef CPURIG_FEATURE_MSR
const char *cpurig::RxConfig::msrPresetName() const
{
    return modNames[msrMod()];
}


const cpurig::MsrItems &cpurig::RxConfig::msrPreset() const
{
...
uint32_t cpurig::RxConfig::msrMod() const
{
    if (!wrmsr()) {
...
void cpurig::RxConfig::readMSR(const rapidjson::Value &value)
{
    if (value.IsBool()) {
...
cpurig::RxConfig::Mode cpurig::RxConfig::readMode(const rapidjson::Value &value)
{
    if (value.IsUint()) {
