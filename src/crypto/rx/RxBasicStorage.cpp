...
{
public:
    CPURIG_DISABLE_COPY_MOVE(RxBasicStoragePrivate)
...
cpurig::RxBasicStorage::RxBasicStorage() :
...
bool cpurig::RxBasicStorage::isAllocated() const
{
    return d_ptr->dataset() && d_ptr->dataset()->cache() && d_ptr->dataset()->cache()->get();
}


cpurig::HugePagesInfo cpurig::RxBasicStorage::hugePages() const
{
    if (!d_ptr->dataset()) {
        return {};
    }
...
cpurig::RxDataset *cpurig::RxBasicStorage::dataset(const Job &job, uint32_t) const
{
...
void cpurig::RxBasicStorage::init(const RxSeed &seed, uint32_t threads, bool hugePages, bool oneGbPages, RxConfig::Mode mode, int priority)
{
    d_ptr->setSeed(seed);
