...
cpurig::HugePagesInfo cpurig::Rx::hugePages()
{
    return d_ptr->queue.hugePages();
}


cpurig::RxDataset *cpurig::Rx::dataset(const Job &job, uint32_t nodeId)
{
    return d_ptr->queue.dataset(job, nodeId);
}


void cpurig::Rx::destroy()
{
#   ifdef CPURIG_FEATURE_MSR
    RxMsr::destroy();
#   endif
...
void cpurig::Rx::init(IRxListener *listener)
{
...
#if defined(CPURIG_FEATURE_AVX2)
...
template<typename T>
bool cpurig::Rx::init(const T &seed, const RxConfig &config, const CpuConfig &cpu)
{
...
#       if defined(CPURIG_FEATURE_SSE4_1)
...
template<typename T>
bool cpurig::Rx::isReady(const T &seed)
{
