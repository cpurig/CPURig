...
{
public:
    CPURIG_DISABLE_COPY_MOVE_DEFAULT(RxNUMAStoragePrivate)
...
cpurig::RxNUMAStorage::RxNUMAStorage(const std::vector<uint32_t> &nodeset) :
    d_ptr(new RxNUMAStoragePrivate(nodeset))
{
}


cpurig::RxNUMAStorage::~RxNUMAStorage()
{
    delete d_ptr;
}


bool cpurig::RxNUMAStorage::isAllocated() const
{
    return d_ptr->isAllocated();
}


cpurig::HugePagesInfo cpurig::RxNUMAStorage::hugePages() const
{
...
cpurig::RxDataset *cpurig::RxNUMAStorage::dataset(const Job &job, uint32_t nodeId) const
{
...
void cpurig::RxNUMAStorage::init(const RxSeed &seed, uint32_t threads, bool hugePages, bool oneGbPages, RxConfig::Mode, int priority)
{
    d_ptr->setSeed(seed);
