...
cpurig::RxCache::RxCache(bool hugePages, uint32_t nodeId)
{
...
cpurig::RxCache::RxCache(uint8_t *memory)
{
    create(memory);
}


cpurig::RxCache::~RxCache()
{
...
bool cpurig::RxCache::init(const Buffer &seed)
{
    if (m_seed == seed) {
        return false;
    }
...
cpurig::HugePagesInfo cpurig::RxCache::hugePages() const
{
    return m_memory ? m_memory->hugePages() : HugePagesInfo();
}


void cpurig::RxCache::create(uint8_t *memory)
{
    if (!memory) {
        return;
    }
