...
cpurig::DmiMemory::DmiMemory(dmi_header *h)
{
    if (h->length < 0x15) {
        return;
    }
...
const char *cpurig::DmiMemory::formFactor() const
{
    return dmi_memory_device_form_factor(m_formFactor);
}


const char *cpurig::DmiMemory::type() const
{
...
rapidjson::Value cpurig::DmiMemory::toJSON(rapidjson::Document &doc) const
{
...
void cpurig::DmiMemory::setId(const char *slot, const char *bank)
{
    m_slot = slot;
    m_bank = bank;
