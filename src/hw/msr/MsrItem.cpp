...
cpurig::MsrItem::MsrItem(const rapidjson::Value &value)
{
    if (!value.IsString()) {
        return;
    }
...
rapidjson::Value cpurig::MsrItem::toJSON(rapidjson::Document &doc) const
{
    return toString().toJSON(doc);
}


cpurig::String cpurig::MsrItem::toString() const
{
