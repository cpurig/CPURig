...
bool cpurig::Config::isPauseOnBattery() const
{
    return d_ptr->pauseOnBattery;
}


const cpurig::CpuConfig &cpurig::Config::cpu() const
{
    return d_ptr->cpu;
}


uint32_t cpurig::Config::idleTime() const
{
...
#ifdef CPURIG_FEATURE_OPENCL
const cpurig::OclConfig &cpurig::Config::cl() const
{
    return d_ptr->cl;
}
#endif
...
#ifdef CPURIG_FEATURE_CUDA
const cpurig::CudaConfig &cpurig::Config::cuda() const
{
    return d_ptr->cuda;
}
#endif
...
#ifdef CPURIG_ALGO_RANDOMX
const cpurig::RxConfig &cpurig::Config::rx() const
{
    return d_ptr->rx;
}
#endif
...
#if defined(CPURIG_FEATURE_NVML) || defined (CPURIG_FEATURE_ADL)
uint32_t cpurig::Config::healthPrintTime() const
{
...
#ifdef CPURIG_FEATURE_DMI
bool cpurig::Config::isDMI() const
{
    return d_ptr->dmi;
}
#endif
...
bool cpurig::Config::isShouldSave() const
{
    if (!isAutoSave()) {
        return false;
    }
...
bool cpurig::Config::read(const IJsonReader &reader, const char *fileName)
{
...
void cpurig::Config::getJSON(rapidjson::Document &doc) const
{
