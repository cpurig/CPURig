...
{
public:
    CPURIG_DISABLE_COPY_MOVE_DEFAULT(MinerPrivate)
...
cpurig::Miner::Miner(Controller *controller)
...
bool cpurig::Miner::isEnabled() const
{
    return d_ptr->enabled;
}


bool cpurig::Miner::isEnabled(const Algorithm &algorithm) const
{
    return std::find(d_ptr->algorithms.begin(), d_ptr->algorithms.end(), algorithm) != d_ptr->algorithms.end();
}


const cpurig::Algorithms &cpurig::Miner::algorithms() const
{
    return d_ptr->algorithms;
}


const std::vector<cpurig::IBackend *> &cpurig::Miner::backends() const
{
    return d_ptr->backends;
}


cpurig::Job cpurig::Miner::job() const
{
...
void cpurig::Miner::execCommand(char command)
{
    switch (command) {
    case 'h':
    case 'H':
...
void cpurig::Miner::setEnabled(bool enabled)
{
...
void cpurig::Miner::setJob(const Job &job, bool donate)
{
...
void cpurig::Miner::onConfigChanged(Config *config, Config *previousConfig)
{
    d_ptr->rebuild();
...
void cpurig::Miner::onTimer(const Timer *)
{
...
#ifdef CPURIG_FEATURE_API
void cpurig::Miner::onRequest(IApiRequest &request)
{
...
#ifdef CPURIG_ALGO_RANDOMX
void cpurig::Miner::onDatasetReady()
{
    if (!Rx::isReady(job())) {
        return;
    }
