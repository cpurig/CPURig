...
void cpurig::BaseTransform::load(JsonChain &chain, Process *process, IConfigTransform &transform)
{
...
void cpurig::BaseTransform::finalize(rapidjson::Document &doc)
{
    using namespace rapidjson;
...
void cpurig::BaseTransform::transform(rapidjson::Document &doc, int key, const char *arg)
{
    switch (key) {
...
#           ifdef CPURIG_FEATURE_TLS
                "stratum+ssl://randomx.cpurig.com:443"
#           else
                "randomx.cpurig.com:3333"
#           endif
            );
        } else
#       endif
...
void cpurig::BaseTransform::transformBoolean(rapidjson::Document &doc, int key, bool enable)
{
    switch (key) {
    case IConfig::BackgroundKey: /* --background */
...
void cpurig::BaseTransform::transformUint64(rapidjson::Document &doc, int key, uint64_t arg)
{
    switch (key) {
    case IConfig::RetriesKey: /* --retries */
