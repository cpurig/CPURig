...
bool cpurig::BaseConfig::read(const IJsonReader &reader, const char *fileName)
{
    m_fileName = fileName;
...
void cpurig::BaseConfig::printVersions()
{
    char buf[256] = { 0 };
...
#   if defined(CPURIG_FEATURE_HWLOC)
...
void cpurig::BaseConfig::setVerbose(const rapidjson::Value &value)
{
    if (value.IsBool()) {
