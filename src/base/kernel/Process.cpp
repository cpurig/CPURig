...
#       endif
    ...
    if (location == Process::ExeLocation) {
        ...
        const auto pos  = path.rfind(*CPURIG_DIR_SEPARATOR);
...
cpurig::Process::Process(int argc, char **argv) :
...
#   ifdef CPURIG_SHARED_DATADIR
    if (dataDir.empty()) {
        dataDir = fmt::format("{}" CPURIG_DIR_SEPARATOR ".cpurig" CPURIG_DIR_SEPARATOR, location(HomeLocation));
        MKDIR(dataDir);
...
cpurig::String cpurig::Process::exepath()
{
    size_t size = sizeof(pathBuf);
...
cpurig::String cpurig::Process::location(Location location, const char *fileName)
{
    ...
    return fmt::format("{}" CPURIG_DIR_SEPARATOR "{}", path, fileName).c_str();
}
