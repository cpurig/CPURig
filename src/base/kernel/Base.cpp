...
#ifdef CPURIG_FEATURE_EMBEDDED_CONFIG
...
{
public:
    CPURIG_DISABLE_COPY_MOVE_DEFAULT(BasePrivate)
...
private:
    inline static Config *load(Process *process)
    {
        ...
        chain.addFile(Process::location(Process::HomeLocation, ".config" CPURIG_DIR_SEPARATOR APP_ID ".json"));
        if (read(chain, config)) {
            return config.release();
        }
...
cpurig::Base::Base(Process *process)
...
bool cpurig::Base::isReady() const
{
    return d_ptr->config != nullptr;
}


int cpurig::Base::init()
{
#   ifdef CPURIG_FEATURE_API
...
cpurig::Api *cpurig::Base::api() const
{
...
bool cpurig::Base::isBackground() const
{
    return d_ptr->config && d_ptr->config->isBackground();
}


bool cpurig::Base::reload(const rapidjson::Value &json)
{
...
cpurig::Config *cpurig::Base::config() const
{
...
void cpurig::Base::addListener(IBaseListener *listener)
{
    d_ptr->listeners.push_back(listener);
}


void cpurig::Base::onFileChanged(const String &fileName)
{
...
#ifdef CPURIG_FEATURE_API
void cpurig::Base::onRequest(IApiRequest &request)
{
