...
cpurig::Arguments::Arguments(int argc, char **argv) :
    m_argv(argv),
    m_argc(argc)
{
...
bool cpurig::Arguments::hasArg(const char *name) const
{
    if (m_argc == 1) {
        return false;
    }
...
const char *cpurig::Arguments::value(const char *key1, const char *key2) const
{
...
void cpurig::Arguments::add(const char *arg)
{
    if (arg == nullptr) {
        return;
    }
