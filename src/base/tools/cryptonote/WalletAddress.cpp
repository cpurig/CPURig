...
bool cpurig::WalletAddress::decode(const char *address, size_t size)
{
...
bool cpurig::WalletAddress::decode(const rapidjson::Value &address)
{
    return address.IsString() && decode(address.GetString(), address.GetStringLength());
}


const char *cpurig::WalletAddress::netName() const
{
...
const char *cpurig::WalletAddress::typeName() const
{
...
rapidjson::Value cpurig::WalletAddress::toJSON(rapidjson::Document &doc) const
{
...
rapidjson::Value cpurig::WalletAddress::toAPI(rapidjson::Document &doc) const
{
    using namespace rapidjson;
...
const cpurig::WalletAddress::TagInfo &cpurig::WalletAddress::tagInfo(uint64_t tag)
{
