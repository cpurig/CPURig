...
cpurig::ConsoleLog::ConsoleLog(const Title &title)
{
    if (!isSupported()) {
        Log::setColors(false);
        return;
...
cpurig::ConsoleLog::~ConsoleLog()
{
    Handle::close(m_tty);
}


void cpurig::ConsoleLog::print(uint64_t, int, const char *line, size_t, size_t size, bool colors)
{
...
bool cpurig::ConsoleLog::isSupported()
{
...
#ifdef CPURIG_OS_WIN
bool cpurig::ConsoleLog::isWritable() const
{
