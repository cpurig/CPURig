...
{
public:
    CPURIG_DISABLE_COPY_MOVE(LogPrivate)
...
void cpurig::Log::add(ILogBackend *backend)
{
    assert(d != nullptr);
...
void cpurig::Log::init()
{
    d = new LogPrivate();
}


void cpurig::Log::print(const char *fmt, ...)
{
    if (!d) {
        return;
    }
...
void cpurig::Log::print(Level level, const char *fmt, ...)
{
    if (!d) {
        return;
    }
