...
bool cpurig::FileLogWriter::open(const char *fileName)
{
...
bool cpurig::FileLogWriter::write(const char *data, size_t size)
{
    if (!isOpen()) {
        return false;
    }
...
bool cpurig::FileLogWriter::writeLine(const char *data, size_t size)
{
