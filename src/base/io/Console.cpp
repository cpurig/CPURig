...
cpurig::Console::Console(IConsoleListener *listener)
    : m_listener(listener)
{
    if (!isSupported()) {
        return;
...
bool cpurig::Console::isSupported()
{
...
void cpurig::Console::onAllocBuffer(uv_handle_t *handle, size_t, uv_buf_t *buf)
{
...
void cpurig::Console::onRead(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf)
{
    if (nread < 0) {
