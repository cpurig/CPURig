...
cpurig::HttpApiRequest::HttpApiRequest(const HttpData &req, bool restricted) :
...
bool cpurig::HttpApiRequest::accept()
{
...
const rapidjson::Value &cpurig::HttpApiRequest::json() const
{
    if (type() == REQ_JSON_RPC) {
...
cpurig::IApiRequest::Method cpurig::HttpApiRequest::method() const
{
    return static_cast<IApiRequest::Method>(m_req.method);
}


void cpurig::HttpApiRequest::done(int status)
{
...
void cpurig::HttpApiRequest::setRpcError(int code, const char *message)
{
    using namespace rapidjson;
    auto &allocator = doc().GetAllocator();
...
void cpurig::HttpApiRequest::setRpcResult(rapidjson::Value &result)
{
    rpcDone(kResult, result);
}


void cpurig::HttpApiRequest::rpcDone(const char *key, rapidjson::Value &value)
{
    ApiRequest::done(0);
