...
cpurig::Algorithm::Algorithm(const rapidjson::Value &value) :
    m_id(parse(value.GetString()))
{
}


cpurig::Algorithm::Algorithm(uint32_t id) :
    m_id(kAlgorithmNames.count(id) ? static_cast<Id>(id) : INVALID)
{
}


const char *cpurig::Algorithm::name() const
{
    if (!isValid()) {
        return kINVALID;
    }
...
rapidjson::Value cpurig::Algorithm::toJSON() const
{
...
rapidjson::Value cpurig::Algorithm::toJSON(rapidjson::Document &) const
{
    return toJSON();
}


cpurig::Algorithm::Id cpurig::Algorithm::parse(const char *name)
{
...
size_t cpurig::Algorithm::count()
{
    return kAlgorithmNames.size();
}


std::vector<cpurig::Algorithm> cpurig::Algorithm::all(const std::function<bool(const Algorithm &algo)> &filter)
{
