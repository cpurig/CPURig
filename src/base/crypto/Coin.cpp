...
cpurig::Coin::Coin(const rapidjson::Value &value)
{
    if (value.IsString()) {
...
cpurig::Algorithm cpurig::Coin::algorithm(uint8_t) const
{
    return coinInfo[m_id].algorithm;
}


const char *cpurig::Coin::code() const
{
    return coinInfo[m_id].code;
}


const char *cpurig::Coin::name() const
{
    return coinInfo[m_id].name;
}


rapidjson::Value cpurig::Coin::toJSON() const
{
...
uint64_t cpurig::Coin::target(uint8_t) const
{
    return coinInfo[m_id].target;
}


uint64_t cpurig::Coin::units() const
{
    return coinInfo[m_id].units;
}


cpurig::Coin::Id cpurig::Coin::parse(const char *name)
{
...
const char *cpurig::Coin::tag(Id id)
{
