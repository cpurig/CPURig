...
cpurig::HttpsContext::HttpsContext(TlsContext *tls, const std::weak_ptr<IHttpListener> &listener) :
...
cpurig::HttpsContext::~HttpsContext() = default;


void cpurig::HttpsContext::append(char *data, size_t size)
{
    if (m_mode == TLS_AUTO) {
...
bool cpurig::HttpsContext::write(BIO *bio)
{
...
void cpurig::HttpsContext::parse(char *data, size_t size)
{
...
void cpurig::HttpsContext::shutdown()
{
    close();
}


void cpurig::HttpsContext::write(std::string &&data, bool close)
{
    m_close = close;
