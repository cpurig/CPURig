...
cpurig::HttpsClient::HttpsClient(const char *tag, FetchRequest &&req, const std::weak_ptr<IHttpListener> &listener) :
...
cpurig::HttpsClient::~HttpsClient()
{
    if (m_ctx) {
        SSL_CTX_free(m_ctx);
    }
...
const char *cpurig::HttpsClient::tlsFingerprint() const
{
    return m_ready ? m_fingerprint : nullptr;
}


const char *cpurig::HttpsClient::tlsVersion() const
{
    return m_ready ? SSL_get_version(m_ssl) : nullptr;
}


void cpurig::HttpsClient::handshake()
{
    m_ssl = SSL_new(m_ctx);
...
void cpurig::HttpsClient::read(const char *data, size_t size)
{
...
void cpurig::HttpsClient::write(std::string &&data, bool close)
{
...
bool cpurig::HttpsClient::verify(X509 *cert)
{
    if (cert == nullptr) {
        return false;
    }
...
bool cpurig::HttpsClient::verifyFingerprint(X509 *cert)
{
...
void cpurig::HttpsClient::flush(bool close)
{
