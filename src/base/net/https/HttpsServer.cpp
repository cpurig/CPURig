...
cpurig::HttpsServer::HttpsServer(const std::shared_ptr<IHttpListener> &listener) :
    m_listener(listener)
{
}


cpurig::HttpsServer::~HttpsServer()
{
...
bool cpurig::HttpsServer::setTls(const TlsConfig &config)
{
...
void cpurig::HttpsServer::onConnection(uv_stream_t *stream, uint16_t)
{
...
void cpurig::HttpsServer::onRead(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf)
{
