...
static const cpurig::String kLocalHost("127.0.0.1");


cpurig::TcpServer::TcpServer(const String &host, uint16_t port, ITcpServerListener *listener) :
...
cpurig::TcpServer::~TcpServer()
{
    Handle::close(m_tcp);
}


int cpurig::TcpServer::bind()
{
    if (!m_version) {
...
void cpurig::TcpServer::create(uv_stream_t *stream, int status)
{
    if (status < 0) {
        return;
    }
...
void cpurig::TcpServer::onConnection(uv_stream_t *stream, int status)
{
