...
cpurig::LineReader::~LineReader()
{
    NetBuffer::release(m_buf);
}


void cpurig::LineReader::parse(char *data, size_t size)
{
...
void cpurig::LineReader::add(const char *data, size_t size)
{
    if (size + m_pos > CPURIG_NET_BUFFER_CHUNK_SIZE) {
...
void cpurig::LineReader::getline(char *data, size_t size)
{
    char *end        = nullptr;
    char *start      = data;
