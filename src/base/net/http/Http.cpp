...
bool cpurig::Http::isEqual(const Http &other) const
{
...
rapidjson::Value cpurig::Http::toJSON(rapidjson::Document &doc) const
{
...
void cpurig::Http::load(const rapidjson::Value &http)
{
    if (!http.IsObject()) {
        return;
    }
...
void cpurig::Http::setPort(int port)
{
