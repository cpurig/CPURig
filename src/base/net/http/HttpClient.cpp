...
cpurig::HttpClient::HttpClient(const char *tag, FetchRequest &&req, const std::weak_ptr<IHttpListener> &listener) :
...
bool cpurig::HttpClient::connect()
{
...
void cpurig::HttpClient::onResolved(const DnsRecords &records, int status, const char *error)
{
    this->status = status;
    m_dns.reset();
...
void cpurig::HttpClient::onTimer(const Timer *)
{
    close(UV_ETIMEDOUT);
}


void cpurig::HttpClient::handshake()
{
...
void cpurig::HttpClient::read(const char *data, size_t size)
{
    if (!parse(data, size)) {
        close(UV_EPROTO);
    }
...
void cpurig::HttpClient::onConnect(uv_connect_t *req, int status)
{
