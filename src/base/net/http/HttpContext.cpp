...
{
public:
    CPURIG_DISABLE_COPY_MOVE_DEFAULT(HttpWriteBaton)
...
cpurig::HttpContext::HttpContext(int parser_type, const std::weak_ptr<IHttpListener> &listener) :
    HttpData(SEQUENCE++),
...
cpurig::HttpContext::~HttpContext()
{
    delete m_tcp;
    delete m_parser;
}
...
void cpurig::HttpContext::write(std::string &&data, bool close)
{
...
bool cpurig::HttpContext::isRequest() const
{
    return m_parser->type == HTTP_REQUEST;
}


bool cpurig::HttpContext::parse(const char *data, size_t size)
{
    if (size == 0) {
        return true;
    }
...
std::string cpurig::HttpContext::ip() const
{
    char ip[46]           = {};
...
uint64_t cpurig::HttpContext::elapsed() const
{
    return Chrono::steadyMSecs() - m_timestamp;
}


void cpurig::HttpContext::close(int status)
{
    if (!get(id())) {
        return;
    }
...
cpurig::HttpContext *cpurig::HttpContext::get(uint64_t id)
{
...
void cpurig::HttpContext::closeAll()
{
    for (auto &kv : storage) {
...
int cpurig::HttpContext::onHeaderField(llhttp_t *parser, const char *at, size_t length)
{
...
int cpurig::HttpContext::onHeaderValue(llhttp_t *parser, const char *at, size_t length)
{
    auto ctx = static_cast<HttpContext*>(parser->data);
...
void cpurig::HttpContext::attach(llhttp_settings_t *settings)
{
...
void cpurig::HttpContext::setHeader()
{
