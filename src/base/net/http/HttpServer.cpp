...
cpurig::HttpServer::HttpServer(const std::shared_ptr<IHttpListener> &listener) :
    m_listener(listener)
{
}


cpurig::HttpServer::~HttpServer()
{
    HttpContext::closeAll();
}


void cpurig::HttpServer::onConnection(uv_stream_t *stream, uint16_t)
{
