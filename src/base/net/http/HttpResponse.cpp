...
cpurig::HttpResponse::HttpResponse(uint64_t id, int statusCode) :
    m_id(id),
...
bool cpurig::HttpResponse::isAlive() const
{
...
void cpurig::HttpResponse::end(const char *data, size_t size)
{
    if (!isAlive()) {
        return;
    }
