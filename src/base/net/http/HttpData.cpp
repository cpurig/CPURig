...
bool cpurig::HttpData::isJSON() const
{
...
const char *cpurig::HttpData::methodName() const
{
    return llhttp_method_name(static_cast<llhttp_method>(method));
}


rapidjson::Document cpurig::HttpData::json() const
{
    if (status < 0) {
...
const char *cpurig::HttpData::statusName(int status)
{
    if (status < 0) {
