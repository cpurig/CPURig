...
const cpurig::DnsRecord &cpurig::DnsRecords::get(DnsRecord::Type prefered) const
{
...
size_t cpurig::DnsRecords::count(DnsRecord::Type type) const
{
    if (type == DnsRecord::A) {
        return m_ipv4.size();
    }
...
void cpurig::DnsRecords::parse(addrinfo *res)
{
    clear();
