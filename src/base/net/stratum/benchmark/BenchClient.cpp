...
cpurig::BenchClient::BenchClient(const std::shared_ptr<BenchConfig> &benchmark, IClientListener* listener) :
    m_listener(listener),
...
cpurig::BenchClient::~BenchClient()
{
    BenchState::destroy();
}


const char *cpurig::BenchClient::tag() const
{
    return Tags::bench();
}


void cpurig::BenchClient::connect()
{
#   ifdef CPURIG_FEATURE_HTTP
...
void cpurig::BenchClient::setPool(const Pool &pool)
{
    m_pool = pool;
}


void cpurig::BenchClient::onBenchDone(uint64_t result, uint64_t diff, uint64_t ts)
{
    m_result    = result;
    m_diff      = diff;
    m_doneTime  = ts;
...
void cpurig::BenchClient::onBenchReady(uint64_t ts, uint32_t threads, const IBackend *backend)
{
    m_readyTime = ts;
    m_threads   = threads;
    m_backend   = backend;
...
void cpurig::BenchClient::onHttpData(const HttpData &data)
{
#   ifdef CPURIG_FEATURE_HTTP
...
void cpurig::BenchClient::onResolved(const DnsRecords &records, int status, const char *error)
{
#   ifdef CPURIG_FEATURE_HTTP
...
bool cpurig::BenchClient::setSeed(const char *seed)
{
    if (!seed) {
        return false;
    }
...
uint64_t cpurig::BenchClient::referenceHash() const
{
...
#ifdef CPURIG_FEATURE_HTTP
void cpurig::BenchClient::onCreateReply(const rapidjson::Value &value)
{
...
void cpurig::BenchClient::onDoneReply(const rapidjson::Value &)
{
    LOG_NOTICE("%s " WHITE_BOLD("benchmark submitted ") CYAN_BOLD("https://cpurig.com/benchmark/%s"), tag(), m_job.id().data());
    printExit();
}


void cpurig::BenchClient::onGetReply(const rapidjson::Value &value)
{
...
void cpurig::BenchClient::resolve()
{
    m_dns = Dns::resolve(BenchConfig::kApiHost, this);
}


void cpurig::BenchClient::send(Request request)
{
...
void cpurig::BenchClient::setError(const char *message, const char *label)
{
...
void cpurig::BenchClient::update(const rapidjson::Value &body)
{
