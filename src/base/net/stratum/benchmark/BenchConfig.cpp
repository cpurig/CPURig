...
#ifndef CPURIG_DEBUG_BENCHMARK_API
const char *BenchConfig::kApiHost   = "api.cpurig.com";
#else
...
cpurig::BenchConfig::BenchConfig(uint32_t size, const String &id, const rapidjson::Value &object, bool dmi, uint32_t rotation) :
...
cpurig::BenchConfig *cpurig::BenchConfig::create(const rapidjson::Value &object, bool dmi)
{
...
rapidjson::Value cpurig::BenchConfig::toJSON(rapidjson::Document &doc) const
{
...
uint32_t cpurig::BenchConfig::getSize(const char *benchmark)
{
    if (!benchmark) {
        return 0;
    }
