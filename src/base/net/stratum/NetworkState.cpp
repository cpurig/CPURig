...
cpurig::NetworkState::NetworkState(IStrategyListener *listener) : StrategyProxy(listener)
{
}


#ifdef CPURIG_FEATURE_API
rapidjson::Value cpurig::NetworkState::getConnection(rapidjson::Document &doc, int version) const
{
...
rapidjson::Value cpurig::NetworkState::getResults(rapidjson::Document &doc, int version) const
{
    using namespace rapidjson;
    auto &allocator = doc.GetAllocator();
...
void cpurig::NetworkState::printConnection() const
{
    if (!m_active) {
...
void cpurig::NetworkState::printResults() const
{
    if (!m_hashes) {
...
const char *cpurig::NetworkState::scaleDiff(uint64_t &diff)
{
...
std::string cpurig::NetworkState::humanDiff(uint64_t diff)
{
...
void cpurig::NetworkState::onActive(IStrategy *strategy, IClient *client)
{
...
void cpurig::NetworkState::onJob(IStrategy *strategy, IClient *client, const Job &job, const rapidjson::Value &params)
{
...
void cpurig::NetworkState::onPause(IStrategy *strategy)
{
...
void cpurig::NetworkState::onResultAccepted(IStrategy *strategy, IClient *client, const SubmitResult &result, const char *error)
{
    add(result, error);
...
uint32_t cpurig::NetworkState::latency() const
{
...
uint64_t cpurig::NetworkState::avgTime() const
{
    if (m_latency.empty()) {
        return 0;
    }
...
uint64_t cpurig::NetworkState::connectionTime() const
{
    return m_active ? ((Chrono::steadyMSecs() - m_connectionTime)) : 0;
}


void cpurig::NetworkState::add(const SubmitResult &result, const char *error)
{
    if (error) {
        m_rejected++;
        return;
