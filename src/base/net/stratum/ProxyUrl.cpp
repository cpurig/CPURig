...
cpurig::ProxyUrl::ProxyUrl(const rapidjson::Value &value)
{
    m_port = 0;
...
const cpurig::String &cpurig::ProxyUrl::host() const
{
    return m_host.isNull() && isValid() ? kLocalhost : m_host;
}


rapidjson::Value cpurig::ProxyUrl::toJSON(rapidjson::Document &doc) const
{
