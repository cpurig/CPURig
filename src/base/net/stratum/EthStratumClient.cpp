...
cpurig::EthStratumClient::EthStratumClient(int id, const char *agent, IClientListener *listener) :
    Client(id, agent, listener)
{
}


int64_t cpurig::EthStratumClient::submit(const JobResult& result)
{
...
void cpurig::EthStratumClient::login()
{
    m_results.clear();
...
void cpurig::EthStratumClient::onClose()
{
    m_authorized = false;
    Client::onClose();
}
...
bool cpurig::EthStratumClient::handleResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
...
void cpurig::EthStratumClient::parseNotification(const char *method, const rapidjson::Value &params, const rapidjson::Value &)
{
...
void cpurig::EthStratumClient::setExtraNonce(const rapidjson::Value &nonce)
{
    if (!nonce.IsString()) {
...
const char *cpurig::EthStratumClient::errorMessage(const rapidjson::Value &error)
{
...
void cpurig::EthStratumClient::authorize()
{
    using namespace rapidjson;
...
void cpurig::EthStratumClient::onAuthorizeResponse(const rapidjson::Value &result, bool success, uint64_t)
{
    try {
        if (!success) {
...
void cpurig::EthStratumClient::onSubscribeResponse(const rapidjson::Value &result, bool success, uint64_t)
{
    if (!success) {
        return;
    }
...
void cpurig::EthStratumClient::subscribe()
{
    using namespace rapidjson;
