...
cpurig::Job::Job(bool nicehash, const Algorithm &algorithm, const String &clientId) :
...
bool cpurig::Job::setSeedHash(const char *hash)
{
...
bool cpurig::Job::setTarget(const char *target)
{
    if (!target) {
        return false;
    }
...
void cpurig::Job::setDiff(uint64_t diff)
{
    m_diff   = diff;
...
void cpurig::Job::setSigKey(const char *sig_key)
{
...
int32_t cpurig::Job::nonceOffset() const
{
...
uint32_t cpurig::Job::getNumTransactions() const
{
...
void cpurig::Job::copy(const Job &other)
{
...
void cpurig::Job::move(Job &&other)
{
    m_algorithm  = other.m_algorithm;
    m_nicehash   = other.m_nicehash;
    m_size       = other.m_size;
...
void cpurig::Job::setSpendSecretKey(const uint8_t *key)
{
...
void cpurig::Job::setMinerTx(const uint8_t *begin, const uint8_t *end, size_t minerTxEphPubKeyOffset, size_t minerTxPubKeyOffset, size_t minerTxExtraNonceOffset, size_t minerTxExtraNonceSize, const Buffer &minerTxMerkleTreeBranch, bool hasViewTag)
{
...
void cpurig::Job::setViewTagInMinerTx(uint8_t view_tag)
{
    memcpy(m_minerTxPrefix.data() + m_minerTxEphPubKeyOffset + 32, &view_tag, 1);
}


void cpurig::Job::setExtraNonceInMinerTx(uint32_t extra_nonce)
{
    memcpy(m_minerTxPrefix.data() + m_minerTxExtraNonceOffset, &extra_nonce, std::min(m_minerTxExtraNonceSize, sizeof(uint32_t)));
}


void cpurig::Job::generateSignatureData(String &signatureData, uint8_t& view_tag) const
{
...
void cpurig::Job::generateHashingBlob(String &blob) const
{
...
void cpurig::Job::generateMinerSignature(const uint8_t* blob, size_t size, uint8_t* out_sig) const
{
    ...
    cpurig::keccak(tmp, static_cast<int>(size), prefix_hash, sizeof(prefix_hash));
    cpurig::generate_signature(prefix_hash, m_ephPublicKey, m_ephSecretKey, out_sig);
}
