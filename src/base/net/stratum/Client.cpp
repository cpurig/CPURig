...
cpurig::Client::Client(int id, const char *agent, IClientListener *listener) :
...
cpurig::Client::~Client()
{
    delete m_socket;
}


bool cpurig::Client::disconnect()
{
    m_keepAlive = 0;
    m_expire    = 0;
    m_failures  = -1;
...
bool cpurig::Client::isTLS() const
{
#   ifdef CPURIG_FEATURE_TLS
...
const char *cpurig::Client::tlsFingerprint() const
{
#   ifdef CPURIG_FEATURE_TLS
...
const char *cpurig::Client::tlsVersion() const
{
#   ifdef CPURIG_FEATURE_TLS
    if (isTLS()) {
...
int64_t cpurig::Client::send(const rapidjson::Value &obj, Callback callback)
{
...
int64_t cpurig::Client::send(const rapidjson::Value &obj)
{
...
int64_t cpurig::Client::submit(const JobResult &result)
{
    if (m_rpcId.isNull()) return 0; // ignore leftout benchmark jobs

#   ifndef CPURIG_PROXY_PROJECT
...
void cpurig::Client::connect(const Pool &pool)
{
    setPool(pool);
    connect();
}
...
void cpurig::Client::deleteLater()
{
    if (!m_listener) {
        return;
    }
...
void cpurig::Client::tick(uint64_t now)
{
...
void cpurig::Client::onResolved(const DnsRecords &records, int status, const char *error)
{
    m_dns.reset();
...
bool cpurig::Client::parseJob(const rapidjson::Value &params, int *code)
{
    if (!params.IsObject()) {
        *code = 2;
        return false;
...
#   endif
    ...
    if (m_jobs == 0) { // https://github.com/cpurig/cpurig/issues/459
        return false;
    }
...
bool cpurig::Client::send(BIO *bio)
{
#   ifdef CPURIG_FEATURE_TLS
    uv_buf_t buf;
...
bool cpurig::Client::verifyAlgorithm(const Algorithm &algorithm, const char *algo) const
{
...
bool cpurig::Client::write(const uv_buf_t &buf)
{
...
int cpurig::Client::resolve(const String &host)
{
...
int64_t cpurig::Client::send(size_t size)
{
...
void cpurig::Client::connect(const sockaddr *addr)
{
...
bool cpurig::Client::parseLogin(const rapidjson::Value &result, int *code)
{
...
void cpurig::Client::parse(char *line, size_t len)
{
    startTimeout();
...
void cpurig::Client::parseExtensions(const rapidjson::Value &result)
{
    m_extensions.reset();
...
void cpurig::Client::parseNotification(const char *method, const rapidjson::Value &params, const rapidjson::Value &)
{
...
void cpurig::Client::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
...
void cpurig::Client::read(ssize_t nread, const uv_buf_t *buf)
{
...
void cpurig::Client::setState(SocketState state)
{
...
void cpurig::Client::startTimeout()
{
    m_expire = 0;
...
bool cpurig::Client::isCriticalError(const char *message)
{
    if (!message) {
        return false;
    }
...
void cpurig::Client::onClose(uv_handle_t *handle)
{
...
void cpurig::Client::onConnect(uv_connect_t *req, int status)
{
...
void cpurig::Client::onRead(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf)
{
