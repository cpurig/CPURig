...
cpurig::Client::Tls::Tls(Client *client) :
    m_client(client)
{
...
bool cpurig::Client::Tls::handshake(const char* servername)
{
    m_ssl = SSL_new(m_ctx);
...
bool cpurig::Client::Tls::send(const char *data, size_t size)
{
...
const char *cpurig::Client::Tls::fingerprint() const
{
    return m_ready ? m_fingerprint : nullptr;
}


const char *cpurig::Client::Tls::version() const
{
    return m_ready ? SSL_get_version(m_ssl) : nullptr;
}


void cpurig::Client::Tls::read(const char *data, size_t size)
{
...
bool cpurig::Client::Tls::send()
{
    return m_client->send(m_write);
}


bool cpurig::Client::Tls::verify(X509 *cert)
{
    if (cert == nullptr) {
...
bool cpurig::Client::Tls::verifyFingerprint(X509 *cert)
{
