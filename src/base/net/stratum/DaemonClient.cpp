...
cpurig::DaemonClient::DaemonClient(int id, IClientListener *listener) :
...
cpurig::DaemonClient::~DaemonClient()
{
    delete m_timer;
    delete m_ZMQSocket;
}
...
void cpurig::DaemonClient::deleteLater()
{
...
bool cpurig::DaemonClient::disconnect()
{
...
bool cpurig::DaemonClient::isTLS() const
{
#   ifdef CPURIG_FEATURE_TLS
    return m_pool.isTLS();
#   else
...
int64_t cpurig::DaemonClient::submit(const JobResult &result)
{
...
void cpurig::DaemonClient::connect()
{
...
void cpurig::DaemonClient::connect(const Pool &pool)
{
    setPool(pool);
    connect();
}
...
void cpurig::DaemonClient::setPool(const Pool &pool)
{
...
void cpurig::DaemonClient::onHttpData(const HttpData &data)
{
    if (data.status != 200) {
        return retry();
    }
...
void cpurig::DaemonClient::onTimer(const Timer *)
{
    if (m_pool.zmq_port() >= 0) {
        m_prevHash = nullptr;
...
void cpurig::DaemonClient::onResolved(const DnsRecords &records, int status, const char* error)
{
    m_dns.reset();
...
bool cpurig::DaemonClient::isOutdated(uint64_t height, const char *hash) const
{
    return m_job.height() != height || m_prevHash != hash || Chrono::steadyMSecs() >= m_jobSteadyMs + m_pool.jobTimeout();
}


bool cpurig::DaemonClient::parseJob(const rapidjson::Value &params, int *code)
{
...
bool cpurig::DaemonClient::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == -1) {
        return false;
    }
...
int64_t cpurig::DaemonClient::getBlockTemplate()
{
    using namespace rapidjson;
    Document doc(kObjectType);
...
int64_t cpurig::DaemonClient::rpcSend(const rapidjson::Document &doc, const std::map<std::string, std::string> &headers)
{
...
void cpurig::DaemonClient::retry()
{
    m_failures++;
...
void cpurig::DaemonClient::send(const char *path)
{
...
void cpurig::DaemonClient::setState(SocketState state)
{
    if (m_state == state) {
        return;
    }
...
void cpurig::DaemonClient::onZMQConnect(uv_connect_t* req, int status)
{
...
void cpurig::DaemonClient::onZMQRead(uv_stream_t* stream, ssize_t nread, const uv_buf_t* buf)
{
...
void cpurig::DaemonClient::onZMQClose(uv_handle_t* handle)
{
...
void cpurig::DaemonClient::onZMQShutdown(uv_handle_t* handle)
{
    DaemonClient* client = getClient(handle->data);
    if (client) {
#       ifdef APP_DEBUG
...
void cpurig::DaemonClient::ZMQConnected()
{
#   ifdef APP_DEBUG
...
bool cpurig::DaemonClient::ZMQWrite(const char* data, size_t size)
{
...
void cpurig::DaemonClient::ZMQRead(ssize_t nread, const uv_buf_t* buf)
{
    if (nread <= 0) {
...
void cpurig::DaemonClient::ZMQParse()
{
#   ifdef APP_DEBUG
    std::vector<char> msg;
#   endif
...
#   endif
    ...
    // Clear previous hash and check daemon height to guarantee that cpurig will call get_block_template RPC later
...
bool cpurig::DaemonClient::ZMQClose(bool shutdown)
{
    if ((m_ZMQConnectionState == ZMQ_NOT_CONNECTED) || (m_ZMQConnectionState == ZMQ_DISCONNECTING)) {
        if (shutdown) {
