...
cpurig::BaseClient::BaseClient(int id, IClientListener *listener) :
    m_listener(listener),
    m_id(id)
{
}
...
void cpurig::BaseClient::setPool(const Pool &pool)
{
    if (!pool.isValid()) {
        return;
    }
...
bool cpurig::BaseClient::handleResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == 1) {
        return false;
    }
...
bool cpurig::BaseClient::handleSubmitResponse(int64_t id, const char *error)
{
