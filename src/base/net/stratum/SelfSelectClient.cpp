...
cpurig::SelfSelectClient::SelfSelectClient(int id, const char *agent, IClientListener *listener, bool submitToOrigin) :
...
cpurig::SelfSelectClient::~SelfSelectClient()
{
    delete m_client;
}


int64_t cpurig::SelfSelectClient::submit(const JobResult &result)
{
    if (m_submitToOrigin) {
...
void cpurig::SelfSelectClient::tick(uint64_t now)
{
    m_client->tick(now);
...
void cpurig::SelfSelectClient::onJobReceived(IClient *, const Job &job, const rapidjson::Value &)
{
    m_job = job;
...
void cpurig::SelfSelectClient::onLogin(IClient *, rapidjson::Document &doc, rapidjson::Value &params)
{
...
bool cpurig::SelfSelectClient::parseResponse(int64_t id, rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == -1) {
        return false;
    }
...
void cpurig::SelfSelectClient::getBlockTemplate()
{
    setState(WaitState);
...
void cpurig::SelfSelectClient::retry()
{
    setState(RetryState);
}


void cpurig::SelfSelectClient::setState(State state)
{
    if (m_state == state) {
        return;
    }
...
void cpurig::SelfSelectClient::submitBlockTemplate(rapidjson::Value &result)
{
    using namespace rapidjson;
    Document doc(kObjectType);
    auto &allocator = doc.GetAllocator();
...
void cpurig::SelfSelectClient::submitOriginDaemon(const JobResult& result)
{
...
void cpurig::SelfSelectClient::onHttpData(const HttpData &data)
{
    if (data.status != 200) {
        return retry();
    }
