...
#if defined CPURIG_ALGO_KAWPOW || defined CPURIG_ALGO_GHOSTRIDER
...
cpurig::Pool::Pool(const char *url) :
...
cpurig::Pool::Pool(const char *host, uint16_t port, const char *user, const char *password, const char* spendSecretKey, int keepAlive, bool nicehash, bool tls, Mode mode) :
...
cpurig::Pool::Pool(const rapidjson::Value &object) :
    m_flags(1 << FLAG_ENABLED),
    m_pollInterval(kDefaultPollInterval),
    m_jobTimeout(kDefaultJobTimeout),
...
#ifdef CPURIG_FEATURE_BENCHMARK
cpurig::Pool::Pool(const std::shared_ptr<BenchConfig> &benchmark) :
...
cpurig::BenchConfig *cpurig::Pool::benchmark() const
{
...
uint32_t cpurig::Pool::benchSize() const
{
...
bool cpurig::Pool::isEnabled() const
{
#   ifndef CPURIG_FEATURE_TLS
    if (isTLS()) {
        return false;
    }
#   endif

#   ifndef CPURIG_FEATURE_HTTP
...
bool cpurig::Pool::isEqual(const Pool &other) const
{
...
cpurig::IClient *cpurig::Pool::createClient(int id, IClientListener *listener) const
{
...
rapidjson::Value cpurig::Pool::toJSON(rapidjson::Document &doc) const
{
...
std::string cpurig::Pool::printableName() const
{
...
void cpurig::Pool::setKeepAlive(const rapidjson::Value &value)
{
    if (value.IsInt()) {
