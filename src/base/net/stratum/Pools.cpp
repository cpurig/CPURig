...
bool cpurig::Pools::isEqual(const Pools &other) const
{
...
int cpurig::Pools::donateLevel() const
{
#   ifdef CPURIG_FEATURE_BENCHMARK
...
cpurig::IStrategy *cpurig::Pools::createStrategy(IStrategyListener *listener) const
{
    if (active() == 1) {
...
rapidjson::Value cpurig::Pools::toJSON(rapidjson::Document &doc) const
{
...
size_t cpurig::Pools::active() const
{
    size_t count = 0;
    for (const Pool &pool : m_data) {
        if (pool.isEnabled()) {
...
void cpurig::Pools::load(const IJsonReader &reader)
{
    m_data.clear();
...
uint32_t cpurig::Pools::benchSize() const
{
#   ifdef CPURIG_FEATURE_BENCHMARK
...
void cpurig::Pools::toJSON(rapidjson::Value &out, rapidjson::Document &doc) const
{
    using namespace rapidjson;
    auto &allocator = doc.GetAllocator();
...
void cpurig::Pools::setDonateLevel(int level)
{
...
void cpurig::Pools::setProxyDonate(int value)
{
    switch (value) {
...
void cpurig::Pools::setRetries(int retries)
{
...
void cpurig::Pools::setRetryPause(int retryPause)
{
