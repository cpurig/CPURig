 ...
 */
cpurig::TlsConfig::TlsConfig(const rapidjson::Value &value)
{
    if (value.IsObject()) {
...
bool cpurig::TlsConfig::generate(const char *commonName)
{
    TlsGen gen;
...
rapidjson::Value cpurig::TlsConfig::toJSON(rapidjson::Document &doc) const
{
...
void cpurig::TlsConfig::setProtocols(const char *protocols)
{
...
void cpurig::TlsConfig::setProtocols(const rapidjson::Value &protocols)
{
    m_protocols = 0;
