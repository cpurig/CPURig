...
cpurig::TlsContext::~TlsContext()
{
    SSL_CTX_free(m_ctx);
}


cpurig::TlsContext *cpurig::TlsContext::create(const TlsConfig &config)
{
    if (!config.isEnabled()) {
        return nullptr;
    }
...
bool cpurig::TlsContext::load(const TlsConfig &config)
{
...
bool cpurig::TlsContext::setCiphers(const char *ciphers)
{
...
bool cpurig::TlsContext::setCipherSuites(const char *ciphersuites)
{
...
bool cpurig::TlsContext::setDH(const char *dhparam)
{
#   if OPENSSL_VERSION_NUMBER < 0x30000000L || defined(LIBRESSL_VERSION_NUMBER)
    DH *dh = nullptr;
...
void cpurig::TlsContext::setProtocols(uint32_t protocols)
{
    if (protocols == 0) {
        return;
    }
