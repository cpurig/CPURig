...
cpurig::ServerTls::ServerTls(SSL_CTX *ctx) :
    m_ctx(ctx)
{
}


cpurig::ServerTls::~ServerTls()
{
    if (m_ssl) {
        SSL_free(m_ssl);
    }
...
bool cpurig::ServerTls::isHTTP(const char *data, size_t size)
{
    assert(size > 0);
...
bool cpurig::ServerTls::isTLS(const char *data, size_t size)
{
    assert(size > 0);
...
bool cpurig::ServerTls::send(const char *data, size_t size)
{
...
void cpurig::ServerTls::read(const char *data, size_t size)
{
    if (!m_ssl) {
        m_ssl = SSL_new(m_ctx);
